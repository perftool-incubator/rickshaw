...
class RunState:
    ...
    def apply_tool_multiplex(self, tool_entry, tool_dir):
        ...
        params through multiplex.py's --flat mode for validation/transform/preset
        ...
        Tools have no sets/include/cartesian-product concept -- they always
        have exactly one implicit parameter set and (per schema/tool-params.json)
        never more than one value per param -- so this uses multiplex's --flat
        mode rather than wrapping into the general sets/global-options document
        benchmarks use. Filtering of disabled params and enforcing "exactly one
        combination" both happen inside multiplex.py itself now; any violation
        surfaces as a non-zero rc below, same as any other multiplex failure.
        """
        ...
        flat_params_file = os.path.join(self.config_dir, f"{tool_id}-flat-params.json")
        ...
        with open(flat_params_file, "w") as f:
            json.dump(tool_entry.get("params", []), f)

        tool_params_out_file = os.path.join(self.config_dir, f"{tool_id}-tool-params-multiplexed.json")
        multiplex_cmd = (
            f"{os.environ.get('MULTIPLEX_HOME', '')}/multiplex.py --flat "
            f"--input {flat_params_file} --output {tool_params_out_file} "
        ...
        multiplexed_params, err = load_json_file(tool_params_out_file)
        if multiplexed_params is None:
            logger.error("[ERROR] could not load multiplexed tool params for '%s': %s", tool_id, err)
            sys.exit(1)

        tool_entry["params"] = multiplexed_params
