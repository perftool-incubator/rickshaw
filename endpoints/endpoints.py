...
def image_created_expiration_gmepoch(value, unit):
    """
    Determine the UTC epoch timetamp that any image created before is expired

    Args:
        value (int): The number of units to consider as teh expiration time post creation
        unit (str): The unit of the value -- either "days" or "weeks"

    Globals:
        None

    Returns:
        gmepoch (int): A UTC epoch timestamp such as 1712949457 from <value> <units> ago.  Any image with a creation
                       date older than this is expired
    """
    if unit == "weeks":
        #       seconds/min  minutes/hour  hours/day  days/week  weeks
        delta = 60           * 60          * 24       * 7        * value
    elif unit == "days":
        #       seconds/min  minutes/jour  hours/day  days
        delta = 60           * 60          * 24       * value
...
def process_pre_deploy_roadblock(roadblock_id = None, endpoint_label = None, roadblock_password = None, deployment_followers = None, roadblock_messages_dir = None, roadblock_timeouts = None, early_abort = None, roadblock_connection_watchdog = None):
    """
...
def process_roadblocks(callbacks = None, roadblock_id = None, endpoint_label = None, endpoint_deploy_timeout = None, roadblock_password = None, new_followers = None, roadblock_messages_dir = None, roadblock_timeouts = None, max_sample_failures = None, engine_commands_dir = None, endpoint_dir = None, early_abort = False, roadblock_connection_watchdog = None):
    """
    Process the beginning and ending roadblocks associated with synchronizing a test
...
def process_bench_roadblocks(callbacks = None, roadblock_id = None, endpoint_label = None, roadblock_password = None, max_sample_failures = None, roadblock_messages_dir = None, roadblock_timeouts = None, engine_commands_dir = None, endpoint_dir = None, roadblock_connection_watchdog = None):
    """
...
def process_options():
    ...
    parser.add_argument("--image-map",
                        dest = "image_map",
                        help = "Path to the image-map.json file mapping benchmarks and tools to container image URLs.",
                        required = False,
                        type = str)
...
def init_settings(settings, args):
    ...
    logger.info("Loading image map")
    settings["misc"]["image-map"] = dict()
    if args.image_map:
        with open(args.image_map, "r") as fh:
            settings["misc"]["image-map"] = json.load(fh)
        for bench, role_map in settings["misc"]["image-map"].items():
            for engine_role, userenv_map in role_map.items():
                for userenv, arch_map in userenv_map.items():
                    for arch, image_info in arch_map.items():
                        logger.info("Adding %s to role %s userenv %s arch %s for bench %s to image-map",
                                    image_info["image"], engine_role, userenv, arch, bench)
...
def get_image(settings, image_role, engine_role, userenv, arch=None):
    ...
    Returns:
        dict: Image info with 'image' key (URL) and optional 'auth-file' key
        or
    ...
    if arch and arch in userenv_map:
        return userenv_map[arch]
    elif arch:
        logger.error("Could not find arch %s in image-map[%s][%s][%s]" % (arch, image_role, engine_role, userenv))
        return None
    else:
        logger.error("No arch specified for image-map lookup of %s / %s / %s" % (image_role, engine_role, userenv))
        return None
...
def get_engine_id_image(settings, role, id, userenv, arch=None):
    ...
    if image_role is not None:
        ...
        if role in bench_map:
            ...
            engine_role = "all"
        image_info = get_image(settings, image_role, engine_role, userenv, arch=arch)
    return image_info
